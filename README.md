...
- `functions/api/admin/feedback.ts` + `app/admin/feedback/` — la coda di review, privata: login con `ADMIN_TOKEN` (Secret del progetto, confronto a tempo costante) che apre una **sessione KV** casuale in un cookie `__Host-` `HttpOnly`/`Secure`/`SameSite=Strict` (il token non viaggia più nel cookie), logout che revoca davvero, rate limit sul login, id di moderazione confinati ai record `fb:` (una sessione non può nominare la coda o un'altra sessione) e tetto del corpo misurato sui byte letti. La pagina non esiste per nessun indice — niente sitemap, niente `llms.txt`, niente feed, niente indice della chat, **niente card `.md`** (era servita come asset statico) — e `scripts/test-admin.mjs` / `scripts/test-feedback.mjs` provano a rompere il login e l'endpoint pubblico offline, perché queste proprietà non si vedono nell'export
- `functions/api/feedback.ts` — Pages Function del form di feedback: honeypot, rate limit KV, validazione, salvataggio in KV `FEEDBACK` con i **primi due ottetti** dell'IP e notifica Resend a `ceo@usepayle.com` il cui link si compone dall'host che ha servito la pagina (non da una costante)
...
npm run test:feedback    # attacchi offline all'endpoint pubblico del feedback (honeypot, tetto del corpo sui byte arrivati, due ottetti di IP, link della notifica sull'host che ha servito la pagina)
