...
- `functions/_middleware.ts` — Pages Function che fa tre cose: sotto `/admin/` risponde **404 a ogni `.md`** (una pagina privata non ha card, e la risposta arriva prima degli asset: l'edge aveva continuato a servire per mezz'ora la copia in cache di `admin/feedback.md` dopo che il file era stato cancellato), `Accept: text/markdown` su una pagina restituisce la sua card `.md` (con `Content-Type: text/markdown`, `x-markdown-tokens` e `Vary: Accept`), e preserva il dominio SEO production dichiarato da `lib/site-origin.ts` senza lasciare che `SITE_URL`, l'host della richiesta o un deploy di anteprima lo cambino. `SITE_URL` resta disponibile alle API che costruiscono link applicativi. Tutto il resto passa agli asset. Vedi §Scoperta per gli agenti.
...
| Dominio SEO production | `lib/site-origin.ts` + `NEXT_PUBLIC_SITE_URL` Cloudflare Pages | il middleware preserva sempre l'host dichiarato; `SITE_URL` è solo applicativo |
...
# il middleware conserva il dominio SEO production dichiarato dal build
