...
- `functions/api/feedback.ts` — Pages Function del form di feedback: honeypot, limite del corpo sui byte realmente letti, controllo same-origin, rate limit KV, validazione, salvataggio in KV `FEEDBACK` senza persistenza dell'IP e notifica Brevo a `ceo@usepayle.com` il cui link si compone dall'host che ha servito la pagina (non da una costante)
