...
import { cmsPageIndex } from "@/lib/generated/cms-copy";
...
export function SiteFooter() {
  ...
  // Le sezioni esistenti hanno una forma senza prefisso per l'inglese, e il
  // footer la usa. Le pagine create dal pannello esistono **solo** sotto
  // `/<lingua>/p/<slug>/`, quindi per loro il prefisso va sempre: senza, il
  // footer linkerebbe una pagina che nessun deploy genera.
  const hrefAlways = (path: string) => `/${locale}${path}`;
  return (
    <footer className="mx-auto max-w-[692px] px-6 pb-10">
      ...
      */}
      ...
      <nav aria-label="Site" className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-1000">
        ...
        {/* Pagine create dal pannello. Una pagina che esiste solo nella sitemap
            ma in nessun link interno e' un'orfana: si puo' trovare, ma non
            raggiungere navigando, e il link check la segnala come mancante. Il
            footer le elenca perche' e' l'unica lista che cresce da sola quando
            dal pannello nasce una pagina. */}
        {cmsPageIndex
          .filter((page) => !page.noindex && page.locales.includes(locale))
          .map((page) => (
            <Link key={page.slug} href={hrefAlways(`/p/${page.slug}/`)}>{page.title}</Link>
          ))}
      </nav>
