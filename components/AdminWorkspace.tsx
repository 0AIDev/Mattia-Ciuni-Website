...
type Tab = "overview" | "analytics" | "applicants" | "feedback" | "jobs" | "settings";
...
function ApplicantsView({ applicants, jobs, loading }: Pick<Props, "applicants" | "jobs" | "loading">) {
  const [openId, setOpenId] = useState<string | null>(null);
  return <div className="space-y-6"><header><p className="text-[10px] uppercase tracking-[.16em] text-[#777]">Hiring</p><h1 className="mt-2 font-serif text-4xl italic text-[#111]">Applicants</h1><p className="mt-2 text-sm text-[#666]">Every application from the careers forms, newest first. The CV stays in storage: this list shows what was written.</p></header>{applicants.length ? applicants.map((applicant) => { const open = openId === applicant.id; return <article key={applicant.id} className="admin-card rounded-[20px] border border-[#e5e5e3] bg-white p-5 sm:p-6"><div className="flex flex-wrap items-baseline justify-between gap-3 text-xs text-[#777]"><span className="text-[#222]">{applicant.full_name}{applicant.email ? ` · ${applicant.email}` : ""}{applicant.email_verified ? " · verified" : ""}</span><time>{new Date(applicant.submitted_at || applicant.created_at || "").toLocaleString()}</time></div><p className="mt-2 text-sm font-medium text-[#111]">{jobTitleOf(jobs, String(applicant.job_slug))}{applicant.country_timezone ? <span className="ml-2 font-normal text-[#777]">{String(applicant.country_timezone)}</span> : null}</p>{applicant.artifact_link ? <p className="mt-1 break-all text-xs"><a href={String(applicant.artifact_link)} target="_blank" rel="noreferrer noopener" className="underline">Artifact</a>{applicant.cv_filename ? <span className="ml-3 text-[#999]">CV: {String(applicant.cv_filename)}</span> : null}</p> : null}{open ? <div className="mt-4 space-y-4 text-sm leading-6 text-[#333]"><p className="whitespace-pre-wrap">{String(applicant.artifact_description || "")}</p><p className="whitespace-pre-wrap">{String(applicant.motivation || "")}</p>{applicant.github_url || applicant.portfolio_url ? <p className="text-xs">{applicant.github_url ? <a href={String(applicant.github_url)} target="_blank" rel="noreferrer noopener" className="mr-4 underline">GitHub</a> : null}{applicant.portfolio_url ? <a href={String(applicant.portfolio_url)} target="_blank" rel="noreferrer noopener" className="underline">Portfolio</a> : null}</p> : null}{applicant.custom_answers && Object.keys(applicant.custom_answers).length ? <div className="space-y-2 border-t border-[#ededeb] pt-3 text-xs text-[#555]"><p className="text-[10px] uppercase tracking-[.14em] text-[#777]">Custom answers</p>{Object.entries(applicant.custom_answers).map(([key, value]) => <p key={key} className="whitespace-pre-wrap"><span className="font-mono text-[11px] text-[#999]">{key}: </span>{String(value)}</p>)}</div> : null}</div> : null}<div className="mt-4"><button type="button" onClick={() => setOpenId(open ? null : applicant.id)} className="border-b border-[#aaa] pb-0.5 text-xs text-[#555]">{open ? "Collapse" : "Read application"}</button></div></article>; }) : <div className="admin-card rounded-[20px] border border-[#e5e5e3] bg-white"><Empty>No applications yet.{loading ? "" : " They appear here as soon as someone completes the apply form."}</Empty></div>}</div>;
}
...
function SettingsView({ onCreateNda }: Pick<Props, "onCreateNda">) {
  ...
  return <div className="space-y-6"><header><p className="text-[10px] uppercase tracking-[.16em] text-[#777]">Workspace</p><h1 className="mt-2 font-serif text-4xl italic text-[#111]">Settings</h1><p className="mt-2 text-sm text-[#666]">The controls that should not be hidden in code.</p></header><div className="grid gap-6 lg:grid-cols-2"><Card title="NDA access"><p className="text-sm leading-6 text-[#555]">Create a one-time private link. The recipient name and email are stored in Supabase; the raw token is never stored.</p><div className="mt-5 space-y-3"><label className="block"><span className="mb-1.5 block text-xs text-[#777]">Full name</span><input value={fullName} onChange={(event) => setFullName(event.target.value)} autoComplete="name" className="admin-form-control w-full rounded-full border border-[#bbb] bg-transparent px-4 py-2.5 text-sm text-[#111] outline-none focus:border-[#111]" /></label><label className="block"><span className="mb-1.5 block text-xs text-[#777]">Email</span><input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" className="admin-form-control w-full rounded-full border border-[#bbb] bg-transparent px-4 py-2.5 text-sm text-[#111] outline-none focus:border-[#111]" /></label><button type="button" onClick={() => void createNda()} disabled={creating || !fullName || !email} className="rounded-full bg-[#111] px-4 py-2.5 text-sm text-white disabled:opacity-50">{creating ? "Creating…" : "Create NDA link"}</button>{error ? <p role="alert" className="text-xs text-[#777]">{error}</p> : null}{link ? <div className="rounded-2xl bg-[#f4f4f2] p-3"><p className="text-xs text-[#777]">Copy this link and send it manually. It expires in 30 days and works once.</p><input readOnly value={link} aria-label="Generated NDA link" className="mt-2 w-full min-w-0 bg-transparent text-xs text-[#111] outline-none" onFocus={(event) => event.currentTarget.select()} /></div> : null}</div></Card><Card title="Site configuration"><div className="space-y-5"><label className="flex items-center justify-between gap-4 border-b border-[#ededed] py-3"><span className="text-sm text-[#333]">Default language of the public site</span><span className="text-xs text-[#777]">English, set in code (lib/site.ts)</span></label><label className="flex items-center justify-between gap-4 border-b border-[#ededed] py-3"><span className="text-sm text-[#333]">Open roles come from</span><span className="text-xs text-[#777]">lib/careers/jobs.ts, seeded into the Job offers tab</span></label></div></Card><Card title="Analytics"><div className="space-y-4 text-sm text-[#555]"><p className="leading-6">Anonymous first-party aggregates are active. GA4 remains consent-gated. Microsoft Clarity records sessions with masked inputs. No IP address or user agent is exposed here.</p><div className="flex justify-between border-t border-[#ededeb] pt-3"><span>Umami</span><span>Connected</span></div><div className="flex justify-between border-t border-[#ededeb] pt-3"><span>Microsoft Clarity</span><span>Connected</span></div><div className="flex justify-between border-t border-[#ededeb] pt-3"><span>Supabase views</span><span>Server-only</span></div></div></Card></div></div>;
}

export function AdminWorkspace(props: Props) {
  const [tab, setTab] = useState<Tab>("overview");
  const pending = useMemo(() => props.records.length, [props.records.length]);
  const applicantCount = useMemo(() => props.applicants.length, [props.applicants.length]);
  return <main id="admin-feedback-page" className="min-h-[100dvh] bg-[#f4f4f2] p-3 font-sans text-[#111] sm:p-4 lg:p-5"><aside className="fixed inset-y-5 left-5 z-20 hidden w-60 flex-col rounded-[24px] border border-[#e5e5e3] bg-white px-5 py-6 shadow-[0_10px_30px_rgba(0,0,0,0.04)] lg:flex"><div className="flex items-center gap-2.5"><span aria-hidden="true" className="admin-logo-black h-6 w-6" /><span className="text-sm font-medium tracking-[-.01em]">Mattia Ciuni</span></div><nav aria-label="Admin" className="mt-12 space-y-1">{tabs.map((item) => <button key={item.id} type="button" onClick={() => setTab(item.id)} className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${tab === item.id ? "bg-[#f1f1ef] font-medium text-[#111]" : "text-[#777] hover:bg-[#f7f7f5] hover:text-[#111]"}`}><span>{item.label}</span>{item.id === "feedback" && pending ? <span className="text-xs text-[#777]">{pending}</span> : null}{item.id === "applicants" && applicantCount ? <span className="text-xs text-[#777]">{applicantCount}</span> : null}</button>)}</nav><div className="mt-auto border-t border-[#ededeb] pt-4"><p className="text-xs text-[#777]">{props.identity?.name}</p><button type="button" onClick={props.onLogout} className="mt-4 rounded-full border border-[#e5e5e3] px-3 py-1.5 text-xs text-[#666] hover:border-[#111] hover:text-[#111]">Log out</button></div></aside><div className="min-h-[calc(100dvh-2.5rem)] rounded-[24px] border border-[#e5e5e3] bg-[#fafaf9] lg:ml-[260px]"><header className="flex min-h-14 items-center justify-between border-b border-[#ededeb] px-5 sm:px-8 lg:px-10"><div className="flex items-center gap-2.5"><span aria-hidden="true" className="admin-logo-black h-6 w-6" /><span className="text-sm font-medium">Mattia Ciuni</span></div><button type="button" onClick={props.onLogout} className="rounded-full border border-[#e5e5e3] px-3 py-1.5 text-xs text-[#666] hover:border-[#111] hover:text-[#111] lg:hidden">Log out</button></header><div className="border-b border-[#ededeb] px-5 py-3 lg:hidden"><nav aria-label="Admin mobile" className="flex gap-2 overflow-x-auto text-sm">{tabs.map((item) => <button key={item.id} type="button" onClick={() => setTab(item.id)} className={`shrink-0 rounded-full px-3 py-1.5 ${tab === item.id ? "bg-[#111] text-white" : "text-[#777] hover:bg-[#f1f1ef]"}`}>{item.label}</button>)}</nav></div><div className="mx-auto max-w-[1380px] p-5 sm:p-8 lg:p-12">{props.error ? <p role="alert" className="mb-6 rounded-2xl border border-[#e5e5e3] bg-white px-4 py-3 text-sm text-[#333]">{props.error}</p> : null}{tab === "overview" ? <Overview analytics={props.analytics} records={props.records} jobs={props.jobs} applicants={props.applicants} /> : null}{tab === "analytics" ? <div className="space-y-6"><header><p className="text-[10px] uppercase tracking-[.16em] text-[#777]">Measurement</p><h1 className="mt-2 font-serif text-4xl italic text-[#111]">Analytics</h1><p className="mt-2 text-sm text-[#666]">Visitors, flow, acquisition, and conversions.</p></header><AnalyticsView analytics={props.analytics} /></div> : null}{tab === "applicants" ? <ApplicantsView applicants={props.applicants} jobs={props.jobs} loading={props.loading} /> : null}{tab === "feedback" ? <FeedbackView records={props.records} loading={props.loading} onCreateTest={props.onCreateTest} onModerate={props.onModerate} /> : null}{tab === "jobs" ? <JobsView jobs={props.jobs} loading={props.loading} onSaveJobs={props.onSaveJobs} /> : null}{tab === "settings" ? <SettingsView onCreateNda={props.onCreateNda} /> : null}</div></div></main>;
}
