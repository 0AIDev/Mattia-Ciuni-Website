...
export const onRequestGet = async ({ request, env }: Context): Promise<Response> => {
  ...
  try {
    const lookup = await supabaseRequest<Array<{ id: string; verification_expires_at: string | null; email_verified: boolean }>>(env, `careers_applications?select=id,verification_expires_at,email_verified&verification_token=eq.${encodeURIComponent(tokenHash)}&limit=1`);
    ...
    return redirectTo(request, env, "/careers/confirmed/");
