...
function MetaItem({ label, value, icon }: { label: string; value: string; icon: "globe" | "clock" | "wallet" | "arrow" }) {
  return <div className="min-w-0 bg-preview-bg px-4 py-3.5 sm:px-5"><div className="flex items-center gap-2 text-xs text-gray-1000"><MetaIcon type={icon} /><span>{label}</span></div><p className="mt-1 break-words text-sm leading-snug text-gray-1200">{value}</p></div>;
}
    ...
    <main id="content" data-career-detail className="pb-20 lg:pb-0"><TableOfContents items={toc} locale={locale} placement="viewport-left" />
      <article className="mx-auto grid w-full max-w-[1040px] gap-10 px-5 py-12 leading-relaxed sm:px-6 sm:py-24 lg:grid-cols-[minmax(0,692px)_280px] lg:items-start lg:gap-16">
        ...
        <aside className="career-sidebar">
          ...
          <div className="rounded-2xl border border-gray-300 bg-preview-bg p-5">
            ...
            {job.status === "open" ? <Link href={`${basePath}/${job.slug}/apply/`} className="group mt-6 flex min-h-11 items-center justify-center gap-2 rounded-full bg-gray-1200 px-5 text-center text-sm font-medium text-gray-background transition-opacity hover:opacity-80"><span>{(locale === "en" ? "Start your application" : locale === "it" ? "Inizia la candidatura" : locale === "fr" ? "Commencer la candidature" : locale === "es" ? "Empezar la candidatura" : "Bewerbung starten")}</span><MinimalArrow /></Link> : null}
          </div>
        </aside>
      </div>
      </article>
      {job.status === "open" ? <div className="career-mobile-apply fixed inset-x-0 bottom-0 z-40 border-t border-gray-300 bg-preview-bg/95 p-3 backdrop-blur"><Link href={`${basePath}/${job.slug}/apply/`} className="group flex min-h-11 items-center justify-center gap-2 rounded-full bg-gray-1200 px-5 text-center text-sm font-medium text-gray-background"><span>{(locale === "en" ? "Start your application" : locale === "it" ? "Inizia la candidatura" : locale === "fr" ? "Commencer la candidature" : locale === "es" ? "Empezar la candidatura" : "Bewerbung starten")}</span><MinimalArrow /></Link></div> : null}
    </main>
  );
}
